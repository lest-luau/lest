...
//!
//! The target place is empty, so there is nothing in the DataModel to
//! `require`. The entrypoint script must therefore inline every module it
//! needs into one self-contained chunk that Open Cloud runs and whose return
//! value (the collector's buffered events) it ships back as JSON. Two sources
...
fn write_module_factory(
    ...
    out.push_str(
        "\tlocal function require (spec)\n\
         \t\tlocal id = __map[spec]\n\
         \t\tif id == nil then\n\
         \t\t\terror('lest cloud bundle: unresolved require(' .. tostring(spec) .. ')', 2)\n\
         \t\tend\n\
