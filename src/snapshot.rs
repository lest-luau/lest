...
/// One snapshot failure, kept in parts rather than pre-rendered: the caller
/// folds it into the owning test's `test_fail` (where the key labels the diff),
/// and the JUnit reporter needs a real `<testcase>` with a `<failure>` element
/// (a prose note is invisible to CI annotation), so it needs the spec, the
/// key, and the body separately.
...
impl SnapshotStore {
    ...
    /// the received value. Loads the spec's `.snap` on first sight. Returns
    /// the failure when the snapshot did not pass (a mismatch diff, or a
    /// duplicate key's explanation) — handed back rather than accumulated so
    /// the caller can attribute it to the test that produced it while that
    /// test's events are still streaming.
    pub fn record(
        &mut self,
        spec: &Path,
        spec_display: &str,
        key: &str,
        received: &str,
    ) -> Result<Option<SnapshotFailure>, ToolError> {
        ...
        if !state.seen.insert(key.to_string()) {
            ...
            return Ok(Some(SnapshotFailure {
...
mod tests {
    ...
    fn different_fails_without_update() {
        ...
        let mut store = SnapshotStore::new(false, false, false);
        let failure = store
            .record(&spec, "math.spec.luau", "adds", "4")
            .unwrap()
            .expect("a mismatch must hand back its failure");
        assert_eq!(failure.key, "adds");
        let summary = store.finish().unwrap();
    ...
    fn duplicate_key_is_reported_as_a_failure() {
        ...
        let spec = spec_in(dir.path());
        let mut store = SnapshotStore::new(false, false, false);
        let first = store.record(&spec, "math.spec.luau", "dupe", "3").unwrap();
        assert!(first.is_none());
        let second = store
            .record(&spec, "math.spec.luau", "dupe", "4")
            .unwrap()
            .expect("the colliding record must hand back its failure");
        let summary = store.finish().unwrap();
        ...
        assert_eq!(summary.written, 1);
        assert_eq!(summary.failed, 1);
        assert!(second.detail.contains("duplicate snapshot key"));
